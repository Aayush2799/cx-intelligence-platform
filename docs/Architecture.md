...
flowchart TB
    subgraph Sources["📁 Source Data (Synthetic CSVs)"]
        S1[customers.csv - 50K rows]
        S2[service_interactions.csv - 350K rows]
        S3[product_usage.csv - 1.1M rows]
        S4[surveys.csv - 30K rows]
    end
    subgraph Bronze["🥉 Bronze Layer - Raw Audit"]
        B1[customers]
        B2[service_interactions]
        B3[product_usage]
        B4[surveys]
    end
    ...
    subgraph Delivery["📊 Delivery Layer"]
        D1[AI/BI Dashboard - 6 tiles]
        D2[Genie Self-Service - NL queries]
        D3[Executive Summary]
    end
    subgraph Orchestration["⚙️ Orchestration & Observability"]
        O1[Databricks Workflows DAG<br/>8 tasks, parallel paths]
        O2[Delta Live Tables<br/>Declarative Gold]
        O3[MLflow Tracking]
        O4[dq_log + pipeline_run_log]
    end
    Sources --> Bronze
    Bronze --> Silver
    Silver --> Gold
    Gold --> Delivery
    Silver -.->|writes| O4
    O1 -.->|orchestrates| Bronze
    O1 -.->|orchestrates| Silver
    O1 -.->|orchestrates| Gold
    O2 -.->|manages| Gold
